...
void page_init(void) {
	...
	struct Page *p;
	u_long count = PADDR(freemem) / BY2PG;
	u_long i;
	for(p = pages,i = 0; i < count;i++,p++){
		p->pp_ref = 1;
	}
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	if((*pgdir_entryp & PTE_V) == 0){
		if(create != 0){
			...
			*pgdir_entryp = page2pa(pp); //将初始化的page *pp转为物理地址赋给二级页表
			*pgdir_entryp = *pgdir_entryp | PTE_V | PTE_D; //更新权限位
		}else{
			*ppte = NULL;
			return 0;
		}
	...
	/* Exercise -E_NO_MEM6: Your code here. (3/3) */
	*ppte = (Pte *)(KADDR(PTE_ADDR(*pgdir_entryp))) + PTX(va);  //先将该物理地址取出table项，再转为虚拟地址得到基地址
	return 0;
}
...
int page_insert(Pde *pgdir, u_int asid, struct Page *pp, u_long va, u_int perm) {
	...
	int ans = pgdir_walk(pgdir, va, 1, &pte);
	if(ans != 0){
		return -E_NO_MEM;
	}
