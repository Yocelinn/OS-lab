...
//extern int barrier_n;
//extern int barrier_max;
//extern int barrier_flag;
//int barrier_list[64]={0};
...
int sys_mem_map(u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm) {
	...
	Pte *pte;
	pp = page_lookup(srcenv->env_pgdir, srcva, &pte);	//这个是这样写吗？我不知道参数Pte **应该填哪个参数？？
	if(pp == NULL){
		return -E_INVAL;
	}
...
int sys_exofork(void) {
	...
	e->barrier_max = curenv->barrier_max;
	e->barrier_n = curenv->barrier_n;
	e->barrier_flag = curenv->barrier_flag;
	if(e->barrier_flag){
		int i;
		for(i = 0;i < e->barrier_n;i++) {
			e->barrier_list[i] = curenv->barrier_list[i];
		}
	}	
...
int sys_set_env_status(u_int envid, u_int status) {
	...
	//只有在改变时需要更新操作，所以改变前的对比来决定
	if(env->env_status != status){
		if(env->env_status != ENV_RUNNABLE){ //说明即将变为runnable！
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	if (srcva != 0) {
		...
		// if(sys_mem_map(e->env_id, srcva, curenv->env_id, e->env_ipc_dstva, perm)){
		// 	return -E_INVAL;
		// }
		...
		if(page_insert(e->env_pgdir, e->env_asid, p, e->env_ipc_dstva, perm) < 0){
			return -E_INVAL;//这是我从insert翻到的，我觉得改try也行
		}
	}
	return 0;
}
...
void sys_barrier_alloc(int n){
	curenv->barrier_n=0;
	curenv->barrier_max = n;
	curenv->barrier_flag=1;
}

void sys_barrier_wait(void){
	if(curenv->barrier_flag == 0) {
		return;
	}
	if(curenv->barrier_max == 0) {
		//说明当前进程并不具有barrier权限
		return;
	}
	curenv->barrier_n ++;
	if(curenv->barrier_n == curenv->barrier_max) {
		//达到最大阻塞数，释放全部被阻塞进程
		curenv->barrier_flag = 0;	//barrier失效
		int i = 0;
		struct Env *env;
		for(;i < curenv->barrier_n-1; i++) {
			
			envid2env(curenv->barrier_list[i],&env,0);
			env->barrier_flag = 0;
			env->barrier_n = 0;
			env->env_status = ENV_RUNNABLE;
			TAILQ_INSERT_TAIL(&env_sched_list, (env), env_sched_link);

			curenv->barrier_list[i] = 0;
		}
		curenv->barrier_n = 0;
		return;
	}

	try(sys_set_env_status(0, ENV_NOT_RUNNABLE));
	curenv->barrier_list[curenv->barrier_n - 1] = curenv->env_id;
	schedule(1);
}
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if(is_illegal_va_range(va, len)) {	//检查va
		return -E_INVAL;
	}
	if(	!((pa+len<0x10000020 && pa>=0x10000000) 
		|| (pa+len<0x13004200 && pa>=0x13000000) 
		|| (pa+len<0x15000200 && pa>=0x15000000)))
	{	//检查pa
		return -E_INVAL;	
	}
