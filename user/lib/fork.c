...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if(perm & PTE_COW == 0){
		user_panic("page is not PTE_COW");
	}
...
static void duppage(u_int envid, u_int vpn) {
	...
	//PTE_D有效，PTE_LIBRARY无效，PTE_COW无效，说明不是共享，未设为写时复制，是可写
	r = 0;
	...
	syscall_mem_map(0, addr, envid, addr, perm);//映射到子进程
	if(r) {
		syscall_mem_map(0, addr, 0, addr, perm);//映射到父进程，其实只修改了权限位
	}
}
