...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		panic_on(syscall_write_dev(&cur_offset, DEV_DISK_ADDRESS+DEV_DISK_OFFSET, 4));
		panic_on(syscall_write_dev(&flag_rw, DEV_DISK_ADDRESS+DEV_DISK_START_OPERATION, 4));
